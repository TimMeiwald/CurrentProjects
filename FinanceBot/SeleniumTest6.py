...
# -*- coding: utf-8 -*-
"""
Created on Sun Feb 14 18:47:11 2021

@author: tim
"""

#The whole shit without using Yahoos search bar because they're too retarded
...
class BrowserNavigator():
    ...
    def __init__(self):
        ...
        try:    
            ...
            self.browser = Firefox(profile,options=options)
...
class webscraperFTSE100HistoricalDaily(NavigateYahooFinance):
    ...
    def downloadOneFTSE100DataFile(self,Ticker):
        ...
        # Example of format https://uk.finance.yahoo.com/quote/BP.L/history?p=BP.L
        try:
            frontURL = "https://uk.finance.yahoo.com/quote/"
            backURL = "/history?.p="
            self.navigateToURL(frontURL + Ticker + backURL + Ticker)
            sleep(1)
            self.randWait(2)
    ...
    def scrapeFTSE100Data(self):
        ...
        #Simple counter to break while loop if failure more than Count times
        FailureCount = 0
        while(len(DataToBeExtracted) != 0):
            ...
            if(self.downloadOneFTSE100DataFile(Ticker) == True):
                ...
                total = end - start 
                remainingTime = (total*len(DataToBeExtracted))/60.
                print("Stock: {} successfully downloaded, {}/100, {} minutes remaining".format(Ticker,100-len(DataToBeExtracted),remainingTime))
            else:
                #If download fails, renavigate tp site in case of ending up
                #somewhere weird
                FailureCount -= 1
                if(FailureCount <= 0):
                    raise Exception("Too many failures to extract data, Terminating script")
